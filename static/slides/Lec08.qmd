...
The tidyverse includes the tibble package that extends data frames to be a bit more modern. The core features of tibbles is to have a nicer printing method as well as being "lazy".
...
The follow examples below are equivalent:
...
Sometimes we want to send our results to an function argument other than first one. In these cases we can refer to the previous result using `_` (as long as the argument is named).
