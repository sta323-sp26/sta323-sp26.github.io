...
Just like `mcparallel`, we can evaluate R expressions without blocking the session:
...
Scoped daemon management using named compute profiles — a cleaner alternative to manually calling `daemons(0)` to clean up.


`with_daemons()` evaluates an expression within a profile:

::: {.xsmall}
```{r}
#| eval: false
daemons(4, .compute = "work")

result = with_daemons("work", {
  mirai_map(1:8, \(x) mean(rnorm(1e6)))[]
})
```
:::


`local_daemons()` activates a profile for the current scope:
...
f = function() {
  local_daemons("work")
  mirai_map(1:4, \(x) mean(rnorm(1e6)))[]
}
```
:::
...
Multiple named profiles (e.g. `"cpu"` and `"gpu"`) can coexist without conflicting
...
:::
::: {.column}
Race returns the index of the first completed task
