...
From the previous example `color`, `shape`, and `alpha` are all **aesthetics** while `size` was a **setting**.
...
Instead of overriding with `labs()`, we can annotate the data so that the label is generated automatically, by attaching a `label` argument to the appropriate column in our data frame.
...
Scales control the mapping from data values to aesthetic values — they determine *how* a variable is translated into color, size, position, etc.

Every aesthetic has a default scale, but we can override it:
...
::: {.aside}
The [scales](https://scales.r-lib.org/) package provides a variety of useful helper functions for formatting axis labels, e.g. `label_comma()`, `label_percent()`, `label_dollar()`, etc.
:::
