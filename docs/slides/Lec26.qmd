...
::: {.xsmall}
```{r}
#| code-line-numbers: "|4|5|11-15|17|19"
...
```{r}
X1 = cbind(1, X)
Xt = torch_tensor(X1, dtype = torch_float())
...
::: {.aside}
We use `dtype = torch_float()` (float32) because that is what the `nn` layers we'll see shortly expect.
:::
...
::: {.aside}
`nnf_mse_loss()` defaults to mean reduction (versus the manual sum we used before), which is why the learning rate is so much larger here.
:::
...
ggplot(tibble(iter = seq_along(losses), loss = losses), aes(iter, loss)) +
  geom_line() +
  scale_y_log10() +
  labs(x = "Iteration", y = "Loss (mean MSE)")
```
...
[`luz`](https://mlverse.github.io/luz/) is a high-level training API built on top of `torch`. It removes most of the training-loop boilerplate we just wrote:
...
::: {.aside}
luz is roughly the R analogue of [PyTorch Lightning](https://lightning.ai/docs/pytorch/stable/) - the same `nn_module` we just built can be fit with luz with no changes.
:::
...
* `setup()` - pick a loss function and optimizer constructor

* `set_hparams()` - arguments passed to the model's `initialize()`

* `set_opt_hparams()` - arguments passed to the optimizer (e.g. `lr`)
...
::: {.xsmall}
```{r}
loss = ((predict(fitted, Xt)$cpu() - yt)^2)$sum()
loss$item()
deviance(lm_fit)
```
:::
...
::: {.xsmall}
```{r}
c(as.numeric(fitted$model$linear$bias),
  as.numeric(fitted$model$linear$weight)) |> round(2)
lm_fit |> coef() |> round(2) |> unname()
```
:::
...
Each layer trades a bit of explicitness for a lot of convenience - and the same stack scales unchanged from this 21-parameter linear regression to networks with millions of parameters.
