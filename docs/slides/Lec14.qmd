...
For this demo we will be using the GitHub REST API to access public data about users and repositories. We will be making unauthenticated requests, which are subject to stricter rate limits (60 requests per hour) but do not require credentials.
...
The information we've been retriving is all public, but the GitHub API also has endpoints that require authentication (e.g. to access private user data, create repositories, etc.). This is why when requesting the `sta323-sp26` repos we get only 2 results (all of the private repos are hidden).

Another example is the `/user` endpoint which returns information about the *authenticated* user. If we try to access this endpoint without authentication, we get an error:
...
* `req_url_query()` - add query parameters to URL (e.g. `GET`, `POST`, etc.)
...
Use `req_error()` to customize this behavior,
