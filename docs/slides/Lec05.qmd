...
R packages are just collections of files - R code, compiled code (C, C++, etc.), data, documentation, and others that live in your library path.
...
* `.rda` files created using `usethis::use_data(obj)` which uses `saveRDS()` (one object per file, recommended)
