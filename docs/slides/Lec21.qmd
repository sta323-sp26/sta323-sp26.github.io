...
We can subset and rename columns, sort results, and limit output using `SELECT`, `ORDER BY`, and `LIMIT`. `DISTINCT` removes duplicate values.
...
|---|---|
| `SEQ_SCAN` | Reads every row of a table sequentially |
| `FILTER` | Applies a `WHERE` condition to rows |
| `PROJECTION` | Computes and selects output columns (`SELECT`) |
| `HASH_JOIN` | Joins two tables by building a hash table on the smaller one |
| `HASH_GROUP_BY` | Groups rows and computes aggregates using a hash table (`GROUP BY`) |
| `UNGROUPED_AGGREGATE` | Computes a single aggregate over all rows (no `GROUP BY`) |
| `CROSS_PRODUCT` | Cartesian product of two inputs (`CROSS JOIN`) |
| `ORDER_BY` | Sorts rows (`ORDER BY`) |
...
::: {.column width='50%'}
```sql
SELECT * FROM '~/Scratch/nyctaxi/yellow_tripdata_2024-01.parquet' LIMIT 5;
```
:::
::: {.column width='50%'}
```sql
SELECT * FROM '~/Scratch/nyctaxi/yellow_tripdata_*.parquet' LIMIT 5;
```
:::
::::
...
::: {.small}
```sql
CREATE VIEW taxi AS SELECT * FROM read_parquet('~/Scratch/nyctaxi/yellow_tripdata_*.parquet');
```
:::
...
```{duckdb, session="taxi"}
.timer on
CREATE VIEW taxi AS SELECT * FROM read_parquet('~/Scratch/nyctaxi/yellow_*.parquet');
```
:::
